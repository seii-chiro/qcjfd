import { deleteOASISInstruction, getOASISInstructions } from "@/lib/oasis-query"
...
export type InstructionDataSourceRecord = {
    id: number;
    no: number;
...
const Instructions = () => {
    ...
    const [recordToEdit, setRecordToEdit] = useState<InstructionDataSourceRecord | null>(null)
    const [searchText, setSearchText] = useState("")
    const [pdfDataUrl, setPdfDataUrl] = useState<string>('');

    const { data: instructions, isLoading: instructionsLoading } = useQuery({
        queryKey: ['OASIS', 'instructions'],
        queryFn: () => getOASISInstructions(token ?? "")
    })

    const deleteInstructionMutation = useMutation({
        mutationFn: (id: number) => deleteOASISInstruction(token ?? "", id),
        onSuccess: () => {
    ...
    const dataSource = [...(instructions?.results || [])]
        .reverse()
        .map((item, index) => ({
            id: item?.id,
            no: index + 1,
    ...
    const filteredDataSource = dataSource?.filter(item => {
        ...
        return (
            ...
            item?.instruction_text?.toLowerCase().includes(searchLower) ||
    ...
    const handleEditOpenModal = (record: InstructionDataSourceRecord) => {
        setIsFormModalOpen(true)
        setRecordToEdit(record)
    }

    const handleDelete = (record: InstructionDataSourceRecord) => {
        Modal.confirm({
            centered: true,
            title: `Delete instruction "${record?.instruction_text}"?`,
            content: 'This action cannot be undone.',
            okText: 'Yes, delete it',
            okType: 'danger',
            cancelText: 'Cancel',
            onOk: async () => {
                deleteInstructionMutation.mutate(record?.id)
            }
        })
    }
    ...
    const columns: ColumnsType<InstructionDataSourceRecord> = [
        {
            title: 'No.',
            dataIndex: 'no',
            key: 'no',
        ...
        {
            title: 'Instruction',
            dataIndex: 'instruction_text',
            key: 'instruction_text',
            sorter: (a, b) => (a.instruction_text || '').toLowerCase().localeCompare((b.instruction_text || '').toLowerCase()),
        },
        {
            title: 'Category',
            dataIndex: 'category',
            ...
            filters: generateFilters(dataSource || [], 'category'),
        ...
        },
        {
            ...
            title: 'Actions',
            key: 'actions',
            render: (_: unknown, record: InstructionDataSourceRecord) => (
    ...
    const handleGeneratePDF = () => {
        ...
        const title = "OASIS Instructions";
    ...
    const handleExportExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "OASIS_Instruction");
        XLSX.writeFile(wb, "OASIS_Instruction.xlsx");
    };
    ...
    const items: MenuProps['items'] = [
        ...
        },
        {
            key: '2',
            label: (
                <CSVLink data={dataSource || []} filename="OASIS_Instruction.csv">
                    Export CSV
                </CSVLink>
            ),
        },
    ...
    return (
        <>
            ...
            <div className="text-3xl font-bold mb-6 text-[#1E365D]">Instructions</div>
