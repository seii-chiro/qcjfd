import { deleteOASISUrgency, getOASISUrgency } from "@/lib/oasis-query"
...
import UrgencyForm from "./forms/UrgencyForm"

export type UrgencyDataSourceRecord = {
  id: number;
  no: number;
  code: string;
  description: string;
...
const Urgency = () => {
  ...
  const [recordToEdit, setRecordToEdit] = useState<UrgencyDataSourceRecord | null>(null)
  const [searchText, setSearchText] = useState("")
  const [pdfDataUrl, setPdfDataUrl] = useState<string>('');

  const { data: urgencies, isLoading: urgenciesLoading } = useQuery({
    queryKey: ['OASIS', 'urgencies'],
    queryFn: () => getOASISUrgency(token ?? "")
  })

  const deleteUrgencyMutation = useMutation({
    mutationFn: (id: number) => deleteOASISUrgency(token ?? "", id),
    onSuccess: () => {
      message.success("Urgency deleted")
  ...
  const dataSource = [...(urgencies?.results || [])]
    .reverse()
    .map((item, index) => ({
      id: item?.id,
      no: index + 1,
  ...
  const handleEditOpenModal = (record: UrgencyDataSourceRecord) => {
    setIsFormModalOpen(true)
    setRecordToEdit(record)
  }

  const handleDelete = (record: UrgencyDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete urgency "${record?.code}"?`,
      content: 'This action cannot be undone.',
      okText: 'Yes, delete it',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: async () => {
        deleteUrgencyMutation.mutate(record?.id)
      }
    })
  }
  ...
  const columns: ColumnsType<UrgencyDataSourceRecord> = [
    {
      title: 'No.',
      dataIndex: 'no',
      key: 'no',
    ...
    },
    {
      ...
      title: 'Actions',
      key: 'actions',
      render: (_: unknown, record: UrgencyDataSourceRecord) => (
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "OASIS_Urgency");
    XLSX.writeFile(wb, "OASIS_Urgency.xlsx");
  };
  ...
  const items: MenuProps['items'] = [
    ...
    },
    {
      key: '2',
      label: (
        <CSVLink data={dataSource || []} filename="OASIS_Urgency.csv">
          Export CSV
        </CSVLink>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="text-3xl font-bold mb-6 text-[#1E365D]">Urgency</div>
