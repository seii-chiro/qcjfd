import { deleteOASISResponseTypes, getOASISResponseTypes } from "@/lib/oasis-query"
...
import ResponseTypeForm from "./forms/ResponseTypeForm"

export type ResponseTypeDataSourceRecord = {
  id: number;
  no: number;
  code: string;
  description: string;
...
const ResponseTypes = () => {
  ...
  const [recordToEdit, setRecordToEdit] = useState<ResponseTypeDataSourceRecord | null>(null)
  const [searchText, setSearchText] = useState("")
  const [pdfDataUrl, setPdfDataUrl] = useState<string>('');

  const { data: responseTypes, isLoading: responseTypesLoading } = useQuery({
    queryKey: ['OASIS', 'responseTypes'],
    queryFn: () => getOASISResponseTypes(token ?? "")
  })

  const deleteResponseTypeMutation = useMutation({
    mutationFn: (id: number) => deleteOASISResponseTypes(token ?? "", id),
    onSuccess: () => {
      message.success("Response type deleted")
      queryClient.invalidateQueries({ queryKey: ['OASIS', 'responseTypes'] })
    },
    onError: (err) => {
  ...
  const dataSource = [...(responseTypes?.results || [])]
    .reverse()
    .map((item, index) => ({
      id: item?.id,
      no: index + 1,
  ...
  const handleEditOpenModal = (record: ResponseTypeDataSourceRecord) => {
    setIsFormModalOpen(true)
    setRecordToEdit(record)
  }

  const handleDelete = (record: ResponseTypeDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete response type "${record?.code}"?`,
      content: 'This action cannot be undone.',
      okText: 'Yes, delete it',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: async () => {
        deleteResponseTypeMutation.mutate(record?.id)
      }
    })
  }
  ...
  const columns: ColumnsType<ResponseTypeDataSourceRecord> = [
    {
      title: 'No.',
      dataIndex: 'no',
      key: 'no',
    ...
    },
    {
      ...
      title: 'Actions',
      key: 'actions',
      render: (_: unknown, record: ResponseTypeDataSourceRecord) => (
  ...
  const handleGeneratePDF = () => {
    ...
    const title = "OASIS Response Type";
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "OASIS_Response_Types");
    XLSX.writeFile(wb, "OASIS_Response_Types.xlsx");
  };
  ...
  const items: MenuProps['items'] = [
    ...
    },
    {
      key: '2',
      label: (
        <CSVLink data={dataSource || []} filename="OASIS_Response_Types.csv">
          Export CSV
        </CSVLink>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="text-3xl font-bold mb-6 text-[#1E365D]">Response Types</div>
