import { deleteOASISSeverity, getOASISSeverity } from "@/lib/oasis-query"
...
export type SeverityDataSourceRecord = {
  id: number;
  no: number;
  code: string;
  description: string;
...
const Severity = () => {
  ...
  const [recordToEdit, setRecordToEdit] = useState<SeverityDataSourceRecord | null>(null)
  const [searchText, setSearchText] = useState("")
  const [pdfDataUrl, setPdfDataUrl] = useState<string>('');

  const { data: severities, isLoading: severitiesLoading } = useQuery({
    queryKey: ['OASIS', 'severities'],
    queryFn: () => getOASISSeverity(token ?? "")
  })

  const deleteSeverityMutation = useMutation({
    mutationFn: (id: number) => deleteOASISSeverity(token ?? "", id),
    onSuccess: () => {
  ...
  const handleEditOpenModal = (record: SeverityDataSourceRecord) => {
    setIsFormModalOpen(true)
    setRecordToEdit(record)
  }

  const handleDelete = (record: SeverityDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete severity "${record?.code}"?`,
  ...
  const columns: ColumnsType<SeverityDataSourceRecord> = [
    {
      title: 'No.',
      dataIndex: 'no',
      key: 'no',
    ...
    },
    {
      ...
      title: 'Actions',
      key: 'actions',
      render: (_: unknown, record: SeverityDataSourceRecord) => (
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "OASIS_Severity");
    XLSX.writeFile(wb, "OASIS_Severity.xlsx");
  };
  ...
  const items: MenuProps['items'] = [
    ...
    },
    {
      key: '2',
      label: (
        <CSVLink data={dataSource || []} filename="OASIS_Severity.csv">
          Export CSV
        </CSVLink>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="text-3xl font-bold mb-6 text-[#1E365D]">Severity</div>
