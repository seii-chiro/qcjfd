import { deleteOASISLanguage, getOASISLanguages } from "@/lib/oasis-query"
...
export type LanguageDataSourceRecord = {
  id: number;
  no: number;
  code: string;
  name: string;
...
const Languages = () => {
  ...
  const [recordToEdit, setRecordToEdit] = useState<LanguageDataSourceRecord | null>(null)
  const [searchText, setSearchText] = useState("")
  const [pdfDataUrl, setPdfDataUrl] = useState<string>('');

  const { data: languages, isLoading: languagesLoading } = useQuery({
    queryKey: ['OASIS', 'languages'],
    queryFn: () => getOASISLanguages(token ?? "")
  })

  const deleteLanguageMutation = useMutation({
    mutationFn: (id: number) => deleteOASISLanguage(token ?? "", id),
    onSuccess: () => {
  ...
  const dataSource = [...(languages?.results || [])]
    .reverse()
    .map((item, index) => ({
      id: item?.id,
      no: index + 1,
  ...
  const handleEditOpenModal = (record: LanguageDataSourceRecord) => {
    setIsFormModalOpen(true)
    setRecordToEdit(record)
  }

  const handleDelete = (record: LanguageDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete language "${record?.code}"?`,
      content: 'This action cannot be undone.',
      okText: 'Yes, delete it',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: async () => {
        deleteLanguageMutation.mutate(record?.id)
      }
    })
  }
  ...
  const columns: ColumnsType<LanguageDataSourceRecord> = [
    {
      title: 'No.',
      dataIndex: 'no',
      key: 'no',
    ...
    },
    {
      ...
      title: 'Actions',
      key: 'actions',
      render: (_: unknown, record: LanguageDataSourceRecord) => (
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "OASIS_Language");
    XLSX.writeFile(wb, "OASIS_Language.xlsx");
  };
  ...
  const items: MenuProps['items'] = [
    ...
    },
    {
      key: '2',
      label: (
        <CSVLink data={dataSource || []} filename="OASIS_Language.csv">
          Export CSV
        </CSVLink>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="text-3xl font-bold mb-6 text-[#1E365D]">Languages</div>
