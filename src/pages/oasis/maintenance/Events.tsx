import { deleteOASISEventType, getOASISEventTypes } from "@/lib/oasis-query"
...
export type EventDataSourceRecord = {
  id: number;
  no: number;
  name: string;
  category: string;
...
const Events = () => {
  ...
  const [recordToEdit, setRecordToEdit] = useState<EventDataSourceRecord | null>(null)
  const [searchText, setSearchText] = useState("")
  const [pdfDataUrl, setPdfDataUrl] = useState<string>('');

  const { data: events, isLoading: eventsLoading } = useQuery({
    queryKey: ['OASIS', 'events'],
    queryFn: () => getOASISEventTypes(token ?? "")
  })

  const deleteEventsMutation = useMutation({
    mutationFn: (id: number) => deleteOASISEventType(token ?? "", id),
    onSuccess: () => {
  ...
  const dataSource = [...(events?.results || [])]
    .reverse()
    .map((item, index) => ({
      id: item?.id,
      no: index + 1,
  ...
  const handleEditOpenModal = (record: EventDataSourceRecord) => {
    setIsFormModalOpen(true)
    setRecordToEdit(record)
  }

  const handleDelete = (record: EventDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete event "${record?.name}"?`,
      content: 'This action cannot be undone.',
      okText: 'Yes, delete it',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: async () => {
        deleteEventsMutation.mutate(record?.id)
      }
    })
  }
  ...
  const columns: ColumnsType<EventDataSourceRecord> = [
    {
      title: 'No.',
      dataIndex: 'no',
      key: 'no',
    ...
    },
    {
      ...
      title: 'Actions',
      key: 'actions',
      render: (_: unknown, record: EventDataSourceRecord) => (
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "OASIS_Events");
    XLSX.writeFile(wb, "OASIS_Events.xlsx");
  };
  ...
  const items: MenuProps['items'] = [
    ...
    },
    {
      key: '2',
      label: (
        <CSVLink data={dataSource || []} filename="OASIS_Events.csv">
          Export CSV
        </CSVLink>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="text-3xl font-bold mb-6 text-[#1E365D]">Events</div>
