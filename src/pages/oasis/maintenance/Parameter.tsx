import { deleteOASISParameterReference, getOASISParameterReference } from "@/lib/oasis-query"
...
export type ParameterReferenceDataSourceRecord = {
    id: number;
    no: number;
    param_name: string;
    param_value: string;
...
const Parameter = () => {
    ...
    const [recordToEdit, setRecordToEdit] = useState<ParameterReferenceDataSourceRecord | null>(null)
    const [searchText, setSearchText] = useState("")
    const [pdfDataUrl, setPdfDataUrl] = useState<string>('');

    const { data: parameters, isLoading: parametersLoading } = useQuery({
        queryKey: ['OASIS', 'parameters'],
        queryFn: () => getOASISParameterReference(token ?? "")
    })

    const deleteParameterMutation = useMutation({
        mutationFn: (id: number) => deleteOASISParameterReference(token ?? "", id),
        onSuccess: () => {
    ...
    const dataSource = [...(parameters?.results || [])]
        .reverse()
        .map((item, index) => ({
            id: item?.id,
            no: index + 1,
    ...
    const filteredDataSource = dataSource?.filter(item => {
        ...
        return (
            item?.param_value?.toLowerCase().includes(searchLower) ||
            item?.param_name?.toLowerCase().includes(searchLower) ||
    ...
    const handleEditOpenModal = (record: ParameterReferenceDataSourceRecord) => {
        setIsFormModalOpen(true)
        setRecordToEdit(record)
    }

    const handleDelete = (record: ParameterReferenceDataSourceRecord) => {
        Modal.confirm({
            centered: true,
            title: `Delete parameter "${record?.param_name}"?`,
            content: 'This action cannot be undone.',
            okText: 'Yes, delete it',
            okType: 'danger',
            cancelText: 'Cancel',
            onOk: async () => {
                deleteParameterMutation.mutate(record?.id)
            }
        })
    }
    ...
    const columns: ColumnsType<ParameterReferenceDataSourceRecord> = [
        {
            title: 'No.',
            dataIndex: 'no',
            key: 'no',
        },
        {
            ...
            key: 'param_name',
            sorter: (a, b) => (a.param_name || '').toLowerCase().localeCompare((b.param_name || '').toLowerCase()),
            filters: generateFilters(dataSource || [], 'param_name'),
            onFilter: (value, record) => record.param_name === value,
        },
        {
            title: 'Parameter Value',
            dataIndex: 'param_value',
            key: 'param_value',
            sorter: (a, b) => (a.param_value || '').toLowerCase().localeCompare((b.param_value || '').toLowerCase()),
        },
        {
            title: 'Description',
        ...
        },
        {
            ...
            title: 'Actions',
            key: 'actions',
            render: (_: unknown, record: ParameterReferenceDataSourceRecord) => (
    ...
    const handleExportExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "OASIS_Parameter");
        XLSX.writeFile(wb, "OASIS_Parameter.xlsx");
    };
    ...
    const items: MenuProps['items'] = [
        ...
        },
        {
            key: '2',
            label: (
                <CSVLink data={dataSource || []} filename="OASIS_Parameter.csv">
                    Export CSV
                </CSVLink>
            ),
        },
    ...
    return (
        <>
            ...
            <div className="text-3xl font-bold mb-6 text-[#1E365D]">Parameters</div>
