...
import WatchlistTypeForm from "./forms/WatchlistTypeForm"
import { generatePDFReport, PDFColumn } from "../oasis/generatePDF"
import { deleteWatchlistType, getWatchlistTypes } from "@/lib/watchlistQueries"
...
const WatchlistType = () => {
    ...
    const { data: types, isLoading: typesLoading } = useQuery({
        queryKey: ['watchlist', 'types'],
        queryFn: () => getWatchlistTypes(token ?? "")
    })

    const deleteWatchlistMutation = useMutation({
        mutationFn: (id: number) => deleteWatchlistType(token ?? "", id),
        onSuccess: () => {
            message.success("Type deleted")
            queryClient.invalidateQueries({ queryKey: ['watchlist', 'types'] })
        },
        onError: () => {
            message.error("Failed to delete watchlist type")
        }
    })

    const dataSource = [...(types?.results || [])]
        .reverse()
        .map((item, index) => ({
            id: item?.id,
            no: index + 1,
    ...
    const handleDelete = (record: tableDataSourceRecord) => {
        Modal.confirm({
            ...
            cancelText: 'Cancel',
            onOk: async () => {
                deleteWatchlistMutation.mutate(record?.id)
            }
        })
    }
    ...
    const handleExportExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Watchlist_Types");
        XLSX.writeFile(wb, "Watchlist_Types.xlsx");
    };
    ...
    const items: MenuProps['items'] = [
        ...
        },
        {
            key: '2',
            label: (
                <CSVLink data={dataSource || []} filename="Watchlist_Types.csv">
                    Export CSV
                </CSVLink>
            ),
        },
    ...
    return (
        <>
            ...
            <div className="text-3xl font-bold mb-6 text-[#1E365D]">Watchlist Type</div>
